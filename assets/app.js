...
let content='';
if(route==='home'){
content=`<section class="hero"><div class="hero-top">${avatar()}<span class="eyebrow">${esc(P.name)} / DRAWING JOURNAL</span></div><h1>${esc(P.subtitle).replace(' ','<br>')}<span aria-hidden="true">.</span></h1><div class="hero-bottom"><p class="hero-description">${esc(P.description)}</p><a class="text-link" href="about.html">この記録について ↗</a></div>${P.heroImage?`<figure class="hero-image"><img src="${safe(P.heroImage)}" alt="${esc(P.heroAlt)}"></figure>`:''}</section><section aria-labelledby="latest"><div class="section-top"><h2 id="latest">LATEST ENTRIES</h2><span>最近の制作記録</span></div><div class="feed">${posts.length?posts.slice(0,4).map(entry).join(''):'<p class="empty">最初の記録を、ここから。</p>'}</div><div class="bottom-link"><a class="text-link" href="archive.html">すべての記録を見る ↗</a></div></section>`;
...
let previous='';
content=heading('Archive.','練習も、途中も、考えたことも。日付をたどる制作記録。')+`<div class="archive-layout"><aside class="archive-side" aria-label="年月で絞り込み"><div class="month-links"><a href="${url(type,'')}" ${!month?'aria-current="true"':''}>すべての月</a></div>${years.map(y=>`<h2 class="year">${esc(y)}</h2><div class="month-links">${dates.filter(d=>d.startsWith(y)).map(d=>`<a href="${url(type,d)}" ${d===month?'aria-current="true"':''}>${months[Number(d.slice(5))-1]} <span>(${posts.filter(p=>p.date.startsWith(d)).length})</span></a>`).join('')}</div>`).join('')}</aside><section class="archive-content" aria-label="投稿一覧"><nav class="filters" aria-label="投稿の種類">${types.map(t=>`<a href="${url(t,month)}" ${t===type?'aria-current="true"':''}>${t}</a>`).join('')}</nav><p class="result-info">${filtered.length} ENTRIES${month?' / '+esc(month.replace('-','.')):''}</p>${filtered.length?filtered.map(p=>{const m=p.date.slice(0,7);const h=m!==previous?`<h2 class="month-title">${months[Number(m.slice(5))-1]} ${m.slice(0,4)}</h2>`:'';previous=m;return h+entry(p);}).join(''):'<div class="empty">この条件の記録はまだありません。</div>'}</section></div>`;
}else if(route==='works'){
